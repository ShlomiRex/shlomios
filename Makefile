...
compile:
# kernel.o
	$(gnu_gcc) -nostdlib -nodefaultlibs -ffreestanding -m32 -g -c "$(kernel_dir)/kernel.cpp" -o "$(build_dir)/kernel.o"
# kernel_entry.o
	nasm "$(bootloader_dir)/kernel_entry.asm" -f elf -o "$(build_dir)/kernel_entry.o"
...
# boot.bin - bootloader
	nasm -i$(bootloader_dir) "$(bootloader_dir)/bootsector.asm" -f bin -o "$(build_dir)/boot.bin"
# everything.bin - bootloader + kernel
	cat "$(build_dir)/boot.bin" "$(build_dir)/full_kernel.bin" > "$(build_dir)/everything.bin"
# zeroes.bin - zeroes
	nasm -i$(bootloader_dir) "$(bootloader_dir)/zeroes.asm" -f bin -o "$(build_dir)/zeroes.bin"
